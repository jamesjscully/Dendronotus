...
%----NEUROMODULATION FROM Si1: TWO SEPARATE MECHANISMS---------------

% MECHANISM 1: Conductance (g) modulation - affects synaptic strength
alpham_g = 0.005;       % Forward rate for conductance modulation
betam_g = 0.0001;       % Backward rate for conductance modulation
scalem_g = (alpham_g - betam_g) / alpham_g;  % Scaling factor
gm = 3;                 % Maximum conductance modulation factor

% MECHANISM 2: Alpha (kinetics) modulation - affects synaptic activation rate
alpham_a = 0.005;       % Forward rate for alpha modulation
betam_a = 0.0001;       % Backward rate for alpha modulation
scalem_a = (alpham_a - betam_a) / alpham_a;  % Scaling factor
gm_a = 3;               % Maximum alpha modulation factor
...
s4 = 0;s12 = 0;s21 = 0;s34 = 0.1;s43 = 0.1;
sm_g = .99;  % Initial value for conductance modulation variable
sm_a = .8;   % Initial value for alpha modulation variable
...
V1new= V1 +step*(4*((0.1*(50-(127*V1/105+8265/105))/(exp((50 - (127*V1/105 ...
    ...
    -g41*(1+gm*sm_g/scalem_g)*(V1-30)*s4/scalex2 ... % Conductance modulation via sm_g; kinetics modulation via sm_a in s4
...
V2new= V2 +step*(4*((0.1*(50-(127*V2/105+8265/105))/(exp((50 - (127*V2/105 ...
    ...
    -g32*(1+gm*sm_g/scalem_g)*(V2-30)*s3/scalex2 ... % Conductance modulation via sm_g; kinetics modulation via sm_a in s3
...
% Separate modulation variables for conductance and alpha
sm_g_new = sm_g+step*(alpham_g*sm_g*(1-sm_g)/(1+exp(-20*(V0+20)))-betam_g*(sm_g-0.0001)); % Conductance modulation
sm_a_new = sm_a+step*(alpham_a*sm_a*(1-sm_a)/(1+exp(-20*(V0+20)))-betam_a*(sm_a-0.0001)); % Alpha modulation

%excitation - uses sm_a for kinetics modulation
s4new =s4+step*(alphax*(1+gm_a*sm_a/scalem_a)*s4*(1-s4)/(1+exp(-20*(V4+20)))-betax*(s4-0.0001));
s3new =s3+step*(alphax*(1+gm_a*sm_a/scalem_a)*s3*(1-s3)/(1+exp(-20*(V3+20)))-betax*(s3-0.0001));
...
y2 = y2new; s0 = s0new; sm_g = sm_g_new; sm_a = sm_a_new; s1 = s1new; s2 = s2new; s3 = s3new; s4 = s4new; s12 = s12new; s21 = s21new; s34 = s34new; s43 = s43new;
