...
V1new= V1 +step*(4*((0.1*(50-(127*V1/105+8265/105))/(exp((50 - (127*V1/105 ...
    ...
    -g41*(V1-30)*s4*(1+gm*sm/scalem)/scalex ... % excitation from si3
...
V2new= V2 +step*(4*((0.1*(50-(127*V2/105+8265/105))/(exp((50 - (127*V2/105 ...
    ...
    -g32*(V2-30)*s3*(1+gm*sm/scalem)/scalex ... % excitation from si3
...
%excitation
s4new =s4+step*(alphax*s4*(1-s4)/(1+exp(-20*(V4+20)))-betax*(s4-0.0001));
s3new =s3+step*(alphax*s3*(1-s3)/(1+exp(-20*(V3+20)))-betax*(s3-0.0001));
%slow inhibition
...
plot(time, ss0/scale1*synscale + offset - 90, 'Color', [0.4, .4, .4], 'LineWidth', 1.5)
...
xlim([30 tmax-0])     % Extended to show labels
