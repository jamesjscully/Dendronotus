...
alpha4 = 0.05; beta4 =  0.008;   scale4=alpha4/(alpha4+beta4);
...
t1=40*1000; % this time is in msec
t2=60*1000; % this time is in msec
...
V0= -44; Ca0=.8; h0 =0; n0 =0; s0=0; x0=0;
...
time=zeros(nt1,1);vv4=time;vv1=time;ss4=time;ss1=time;Caa4=time; Caa1=time;xx4=time;xx1=time; vv3=time;vv2=time;...
ss3=time;ss2=time;Caa3=time; Caa2=time;xx3=time;xx2=time; ss12=time; ss21=time; ss34=time; ss43=time;Caa0=time;xx0=time;vv0=time;ss0=time;
...
V0= V0 +step*(Iapp*heaviside(tt-t1)*heaviside(t2-tt)+4*((0.1*(50-(127*V0/105+8265/105))/(exp((50 - (127*V0/105 ...
...
V1= V1 +step*(0*Iapp*heaviside(tt-t1)*heaviside(t2-tt)+4*((0.1*(50-(127*V1/105+8265/105))/(exp((50 - (127*V1/105 ...
    ...
    -g41*(V1-30)*s4/scale4*(1+0*s0/scale0)  -g12*(V1+80)*s21/scale12  +gelec*(V4-V1)+rnd2(i));

V2= V2 +step*(0*Iapp*heaviside(tt-t1)*heaviside(t2+50-tt)+4*((0.1*(50-(127*V2/105+8265/105))/(exp((50 - (127*V2/105 ...
    ...
    -g32*(V2-30)*s3/scale3*(1+0*s0/scale0) -g12*(V2+80)*s12/scale12 +gelec*(V3-V2)+rnd2(i)); 
...
V3 =V3 +step*(4*((0.1*(50-(127*V3/105+8265/105))/(exp((50 - (127*V3/105 ...
    ...
    -g23*(V3+80)*s2/scale2 -g34*(V3+80)*s43/scale34  +gelec*(V2-V3)+rnd1(i)  -g0*(V3-30)*s0/scale0) ;
 ...
 V4 =V4 +step*(4*((0.1*(50-(127*V4/105+8265/105))/(exp((50 - (127*V4/105 ...
    ...
    -g14*(V4+80)*s1/scale1 -g34*(V4+80)*s34/scale34 +gelec*(V1-V4)+rnd1(i)   -g0*(V4-30)*s0/scale0);
...
plot(time, ss1/scale1*synscale + offset - 75, 'Color', [0, 0, 0.6], 'LineWidth', 1.5)
...
plot(time, ss0/scale0*synscale + offset - 90, 'Color', [0.4, .4, .4], 'LineWidth', 1.5)
...
xlim([26 tmax-10])     % Extended to show labels
ylim([65 720])         % Adjusted to fit all traces
...
%period 10=12 sec
exportgraphics(fig, 's1_Fig3A_drives_cpg1.jpg', 'Resolution', 75,'ContentType', 'image', 'BackgroundColor', 'white')
