...
burst_frequencies = zeros(size(gm_values)); % Store burst frequencies
...
burst_threshold = -20; % Voltage threshold for detecting spikes/bursts
min_burst_duration = 100; % Minimum duration in msec to count as burst
min_interburst_interval = 500; % Minimum time between bursts in msec

% Loop over different gm values
fprintf('Running simulations for different gm values...\n');
for gm_idx = 1:length(gm_values)
    gm = gm_values(gm_idx);
    fprintf('Running simulation %d/%d: gm = %.4f\n', gm_idx, length(gm_values), gm);
    
    % Initial values (reset for each simulation)
    V0 = -44; V1 = -44; V2 = -44; V3 = -44; V4 = -44; 
    x0 = 0; x1 = .6; x2 = .6; x3 = .5; x4 = .6;
    Ca0 = .3; Ca1 = 1.; Ca2 = 1.; Ca3 = 1.1; Ca4 = 1.; 
    h0 = 0; h1 = 0; h2 = 0; h3 = 0; h4 = 0; n0 = 0; n1 = 0; n2 = 0; n3 = 0; n4 = 0; y1 = 0; y2 = 0; s0 = 0;s1 = 0;s2 = 0;s3 = 0;
    s4 = 0;s12 = 0;s21 = 0;s34 = 0.1;s43 = 0.1;sm = .99;
    ...
    [peaks, peak_locs] = findpeaks(vv4, 'MinPeakHeight', burst_threshold, 'MinPeakDistance', round(min_interburst_interval/tsamp));
    
    if length(peak_locs) > 1
        % Calculate burst frequency (bursts per second)
        analysis_start_time = 50; % Start analysis after 50 seconds to avoid initial transients
        analysis_end_time = tmax - 10; % End 10 seconds before simulation end
        
        % Find peaks in analysis window
        analysis_indices = find(time(peak_locs) >= analysis_start_time & time(peak_locs) <= analysis_end_time);
        num_bursts = length(analysis_indices);
        analysis_duration = analysis_end_time - analysis_start_time;
        
        burst_frequencies(gm_idx) = num_bursts / analysis_duration;
    else
        burst_frequencies(gm_idx) = 0; % No bursts detected
    end
    
    fprintf('  Detected %d bursts, frequency = %.3f Hz\n', num_bursts, burst_frequencies(gm_idx));
end

% Create scatterplot
figure('Position', [100, 100, 800, 600]);
scatter(gm_values, burst_frequencies, 100, 'filled', 'MarkerFaceColor', [0.2, 0.4, 0.8], 'MarkerEdgeColor', 'k', 'LineWidth', 1.5);
hold on;

% Add trend line if there's a clear relationship
if length(gm_values) > 2
    p = polyfit(gm_values, burst_frequencies, 1);
    trend_line = polyval(p, gm_values);
    plot(gm_values, trend_line, '--r', 'LineWidth', 2);
end

% Formatting
xlabel('gm Parameter Value', 'FontSize', 14, 'FontWeight', 'bold');
ylabel('Burst Frequency (Hz)', 'FontSize', 14, 'FontWeight', 'bold');
title('Effect of gm Parameter on Network Burst Frequency', 'FontSize', 16, 'FontWeight', 'bold');
grid on;
grid minor;
set(gca, 'FontSize', 12);
box on;

% Add text box with correlation coefficient if trend line was added
if length(gm_values) > 2
    R = corrcoef(gm_values, burst_frequencies);
    correlation = R(1,2);
    text(0.05, 0.95, sprintf('Correlation: %.3f', correlation), 'Units', 'normalized', ...
         'FontSize', 12, 'BackgroundColor', 'white', 'EdgeColor', 'black');
end

% Display results table
fprintf('\n=== RESULTS SUMMARY ===\n');
fprintf('gm Value\tBurst Frequency (Hz)\n');
fprintf('--------\t----------------\n');
for i = 1:length(gm_values)
    fprintf('%.4f\t\t%.4f\n', gm_values(i), burst_frequencies(i));
end

% Save results
results_table = table(gm_values', burst_frequencies', 'VariableNames', {'gm_parameter', 'burst_frequency_Hz'});
writetable(results_table, 'gm_burst_analysis_results.csv');
