...
const PRESYNAPTIC_PARAMS = updated_params(
    default_params();
...
const PRESYNAPTIC_SPEC = ModeSweepSpec(
    presynaptic,
    "Presynaptic",
...
function generate_presynaptic_data()
    ...
    @threads for idx in eachindex(gains)
        ...
        summary_parts[idx] = publication_summary_row(sim, run_raw, gain, PRESYNAPTIC_SPEC.mode)
    end
