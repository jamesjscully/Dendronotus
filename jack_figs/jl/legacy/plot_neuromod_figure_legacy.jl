...
function transition_summary_rows(summary::DataFrame, phase::String)
    sorted_summary = sort(summary, :control_gain)
    count_col = phase == "pre" ? :n_pre_intervals : :n_post_intervals
    valid_idx = findall(>(0), sorted_summary[!, count_col])
    if isempty(valid_idx)
        isempty(sorted_summary) && return sorted_summary
        return sorted_summary[[1, nrow(sorted_summary)], :]
    end

    keep = copy(valid_idx)
    prior = findall(i -> i < first(valid_idx), axes(sorted_summary, 1))
    !isempty(prior) && pushfirst!(keep, prior[end])
    return sorted_summary[sort(unique(keep)), :]
end

function representative_summary_row(summary::DataFrame, mode_name::String)
    subset = summary[summary.mode .== mode_name, :]
    isempty(subset) && error("Missing summary rows for $(mode_name)")
    hasproperty(subset, :anchor_gain) || error("Missing anchor_gain column for $(mode_name); regenerate calibrated scan data.")
    anchor_gain = Float64(subset[1, :anchor_gain])
    idx = findall(g -> isapprox(Float64(g), anchor_gain; atol = 1e-10, rtol = 1e-10), subset.control_gain)
    isempty(idx) && error("Representative gain $(anchor_gain) is absent from $(mode_name) scan data.")
    return subset[idx[1], :]
end
...
function plot_gain_panel!(ax::Axis, summary::DataFrame, raw::DataFrame, phase::String)
    ...
    for mode_name in ["presynaptic", "postsynaptic"]
        ...
        sub_summary = transition_summary_rows(summary[summary.mode .== mode_name, :], phase)
        ...
        rep = representative_summary_row(summary, mode_name)
        rep_y = Float64(rep[ycol])
        isfinite(rep_y) || error("Representative $(mode_name) $(phase) frequency is not finite.")
        rep_sc = GLMakie.scatter!(
            ax,
            [Float64(rep.control_gain)],
            [rep_y];
            color = :gold,
            marker = :star5,
            markersize = 22,
        ...
        if mode_name == "presynaptic"
            push!(legend_entries, rep_sc)
            push!(legend_labels, "shown trace gain")
        end
    end
