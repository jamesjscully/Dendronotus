...
const DASHBOARD_SNAPSHOT_PNG = joinpath(DASHBOARD_OUTPUT_DIR, "neuromod_dashboard_snapshot.png")
...
function main(args = ARGS)
    ...
    if mode == "interactive"
        GLMakie.display(fig)
        return
    elseif mode == "snapshot" || mode == "headless"
        CairoMakie.activate!()
        CairoMakie.save(DASHBOARD_SNAPSHOT_PNG, fig; backend = CairoMakie)
        println("Saved $(DASHBOARD_SNAPSHOT_PNG)")
        return
    end

    error("Unknown mode `$(mode)`. Use `interactive`, `snapshot`, or `headless`.")
end
