...
The present work addresses a distinct question: in circuits lacking intrinsic pacemakers, how do slow synaptic kinetics regulate rhythmic output and enable graded frequency control?
...
\subsection*{Neuromodulation of the Si3$\to$Si2 pathway provides frequency control}

The neuromodulation analysis in Fig.~\ref{fig:si1_neuromod} shows that burst frequency can be regulated during sustained drive by modulating the Si3$\to$Si2 excitatory pathway.
Using the same dimensionless gain parameter $g_m$, we compared two loci for that modulation: presynaptic scaling of the logistic accumulation-rate parameter $\alpha$ and postsynaptic scaling of effective synaptic conductance.
Both mechanisms alter burst frequency across the gain scan, but they do so with different gain-to-frequency relationships.
...
Our results extend this principle: in network oscillators lacking cellular pacemakers, frequency control naturally operates through synaptic mechanisms, and the \textit{kinetics} of transmission can materially influence how modulatory gain is translated into network output.
...
Our model therefore motivates experiments that distinguish where Si1-linked modulation acts within the excitatory pathway, rather than supporting a strong claim that one locus is already established as the better biological explanation.
...
Our results are consistent with this view: modulating the Si3$\to$Si2 pathway changes burst frequency during ongoing drive, and the detailed mapping from modulatory gain to burst output depends on whether gain acts on synaptic kinetics or conductance (Fig.~\ref{fig:si1_neuromod}B,C).
...
Adaptability arises because the synaptic locus of the slow dynamics places frequency control within reach of neuromodulatory systems that target synaptic transmission \cite{NadimBucher2014}.
...
Slow synaptic kinetics provide an effective substrate for such regulation: their extended timescales create a temporal buffer that shapes the translation of modulatory signals into network output while preserving coordinated movement.
...
Neuromodulatory control over synaptic transmission may therefore represent a general strategy for achieving reversible frequency control across these systems \cite{NadimBucher2014}.
...
Although the model captures graded frequency control during active drive, initiation and termination of swim episodes are still only approximate, leaving room for improvement in how command-neuron recruitment and release are represented.
