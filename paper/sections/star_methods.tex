...
In both formulations, the same dimensionless gain parameter $g_m$ sets the strength of modulation.
Both mechanisms increase effective synaptic drive, but they do so by acting on different components of the synaptic model.
...
Throughout this analysis, ``gain'' refers to the dimensionless parameter $g_m$ that scales the effect of the modulatory variable $s_m$.
In the presynaptic condition, gain multiplied the logistic accumulation-rate parameter $\alpha$.
In the postsynaptic condition, the same gain multiplied the effective synaptic conductance.
...
To compare model output with experiment, we used the Si1 and Si2 traces digitized from preparation 130618-03 together with the published Si1-versus-burst summary data from Sakurai \& Katz (2019) \cite{sakurai2019command}.
...
This produced paired measurements of Si1 spike frequency and burst frequency for both the biological traces and the representative model traces shown in Fig.~\ref{fig:si1_neuromod}D.
...
The representative presynaptic and postsynaptic traces in Fig.~\ref{fig:si1_neuromod}D were generated by replaying the same digitized Si1 voltage trace into both model variants.
